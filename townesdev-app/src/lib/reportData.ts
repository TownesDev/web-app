...
export async function collectMonthlyReportData(
  ...
  console.log(
    `[Report Data] Collecting data for client ${clientId}, month ${month}`
  )
  ...
  console.log(
    `[Report Data] Collected: ${incidents.length} incidents, ${invoices.length} invoices`
  )
...
function getMonthDateRange(month: string): { startDate: Date; endDate: Date } {
  ...
  if (monthIndex === -1) {
    throw new Error(
      `Invalid month format: ${month}. Expected format: "January 2024"`
    )
  }
...
function generateReportSummary(
  ...
  const criticalIncidents = incidents.filter(
    (inc) => inc.severity === 'urgent' || inc.severity === 'critical'
  ).length

  const incidentHours = incidents.reduce(
    (sum, inc) => sum + (inc.hoursUsed || 0),
    0
  )

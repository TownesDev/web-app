...
export function processEmailAttachments(
  ...
  console.log(
    `[Attachment Processing] Processing ${attachments.length} attachments`
  )
  ...
  return attachments.map((attachment, index) => {
    try {
      ...
      console.error(
        `[Attachment Processing] Error processing ${attachment.filename}:`,
        error
      )
      return {
 ...
 */
export function generateAttachmentSummary(processed: ProcessedAttachment[]): {
  totalCount: number
  totalSize: string
  safeCount: number
  imageCount: number

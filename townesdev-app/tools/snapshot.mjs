...
const list = (dir, depth=2) => {
  ...
  const walk = (d, lvl) => {
    ...
    for (const f of fs.readdirSync(d)) {
      if (["node_modules",".git",".next",".vercel",".vscode"].includes(f)) continue;
...
const pages = ["app/page.tsx","app/layout.tsx","app/(app)/app/page.tsx","app/(app)/app/layout.tsx","app/(admin)/admin/page.tsx"];
push("Key pages/layouts", pages.map(take).filter(Boolean).join("\n\n"));

const envKeys = new Set();
for (const file of fs.readdirSync(".", {withFileTypes:true})) {
  if (file.isDirectory() && ["node_modules",".git",".next"].includes(file.name)) continue;
}
const scanEnv = (dir) => {
  for (const f of fs.readdirSync(dir, {withFileTypes:true})) {
    if (["node_modules",".git",".next",".vercel",".vscode"].includes(f.name)) continue;
    const p = path.join(dir, f.name);
    if (f.isDirectory()) scanEnv(p);
    else if (/\.(t|j)sx?$/.test(f.name)) {
      const txt = slurp(p);
      for (const m of txt.matchAll(/process\.env\.([A-Z0-9_]+)/g)) envKeys.add(m[1]);
    }
  }
};
scanEnv(".");
...
const sanityDir = fs.existsSync("sanity") ? "sanity" : (fs.existsSync("studio") ? "studio" : null);
if (sanityDir) {
  const schemaFiles = [];
  const walk = d => {
  ...
  for (const f of ["schema.ts","schemas.ts","schemas/index.ts","schemaTypes/index.ts"]) {
